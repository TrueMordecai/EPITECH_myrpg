...
static char *const FULL_NAME[] = {
    ...
    "The Crazy Salmon", "CrossBowBoy Special Gift", "Big Aymeric"
...
extern void rpg_create_item_name_generator(item_t *item, int level)
{
    ...
    if (level == 10 && item->rarity == LEGENDARY) {
        my_strcpy(item->name, FULL_NAME[rand() % array_lenght(FULL_NAME)]);
        return;
    }
    index += my_strcpy(item->name, PREFIX[rand() % array_lenght(PREFIX)]);
    index += my_strcpy(&item->name[index], NAME[item->elem * 5 + item->rarity]);
    my_strcpy(&item->name[index], SUFFFIX[rand() % array_lenght(SUFFFIX)]);
}
