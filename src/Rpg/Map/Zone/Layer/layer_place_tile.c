...
static void init_rect(layer_t *layer, int pos_id, sfVector2f pos, int tile)
{
    ...
    sfRectangleShape_setTextureRect(rect,
        (sfIntRect){(tile % SET_WIDTH) * TILE_SIZE,
            (tile / SET_WIDTH) * TILE_SIZE, TILE_SIZE, TILE_SIZE});
    sfRectangleShape_setOutlineColor(rect, sfColor_fromRGB(50, 50, 50));
    sfRectangleShape_setSize(
        rect, (sfVector2f){layer->tiles_size, layer->tiles_size});
...
void layer_place_tile(layer_t *layer, sfVector2i pos, int tile)
{
    if (pos.x < layer->pos.x || pos.y < layer->pos.y
        ...
        return;
    init_rect(layer,
        pos.x - layer->pos.x + (pos.y - layer->pos.y) * layer->size.x,
        (sfVector2f){pos.x * layer->tiles_size, pos.y * layer->tiles_size},
        tile);
}
