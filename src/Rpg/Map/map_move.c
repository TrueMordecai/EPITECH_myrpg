...
static sfVector2i get_size(
    battle_t *battle, sfVector2f top_left, sfVector2f br)
{
...
{
    sfVector2i size_diff = {
        size.x - battle->size.x * M_TO_PX, size.y - battle->size.y * M_TO_PX};
...
static sfVector2f get_offset(
    zone_t *zone, sfVector2f top_left, sfVector2f br, float dt)
{
    ...
    sfVector2f diffs_br = {(br.x > pos_battle.x + size_battle.x)
            ? (pos_battle.x + size_battle.x) - br.x
            : 0,
        (br.y > pos_battle.y + size_battle.y)
            ? (pos_battle.y + size_battle.y) - br.y
            : 0};
    sfVector2f offset = {0, 0};

    offset.x =
        (fabsf(diffs_tl.x) > fabsf(diffs_br.x)) ? diffs_tl.x : diffs_br.x;
    offset.y =
        (fabsf(diffs_tl.y) > fabsf(diffs_br.y)) ? diffs_tl.y : diffs_br.y;
