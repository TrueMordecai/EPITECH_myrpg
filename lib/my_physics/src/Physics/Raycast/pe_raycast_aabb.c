...
pe_raycast_t *pe_raycast_aabb(pe_aabb_t *box, pe_raycast_t *ray)
{
    ...
    pe_vec2f_t near_times = {
        (box->min.x - signs.x * h_size.x - ray->p1.x) * scale.x,
        (box->min.y - signs.y * h_size.y - ray->p1.y) * scale.y};
    pe_vec2f_t far_times = {
        (box->min.x + signs.x * h_size.x - ray->p1.x) * scale.x,
        (box->min.y + signs.y * h_size.y - ray->p1.y) * scale.y};
    ...
    ray->collision_point = VEC2F_ADD(
        ray->p1, VEC2F_MUL(ray->delta, VEC2F(ray->time_hit, ray->time_hit)));
    return ray;
}
