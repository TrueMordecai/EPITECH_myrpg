...
static pe_vec2f_t get_support_point(
    pe_vec2f_t dir, pe_vec2f_t *vertices, int count)
{
...
static float get_penetration_dist(
    pe_mat22_t *mat, pe_fixture_t *a, pe_fixture_t *b, int i)
{
    ...
    pe_vec2f_t vertex_world =
        VEC2F_SUB(VEC2F_ADD(SHAPE_POS(a),
                      pe_mat22_get_rotated_point(
                          &a->shape.mat_rot, poly_a->vertices[i])),
            SHAPE_POS(b));
...
float find_axis_least_penetration(
    int *face_id, pe_fixture_t *a, pe_fixture_t *b)
{
    pe_polygon_shape_t *poly_a = &a->shape.shape.polygon;
