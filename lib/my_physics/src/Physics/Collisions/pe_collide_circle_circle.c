...
char pe_collide_circle_circle(pe_shape_t *c1, pe_shape_t *c2)
{
    ...
    float square_dist =
        powf(c1->pos.x + c2->pos.x, 2) + powf(c1->pos.y + c2->pos.y, 2);
...
char pe_manifold_fill_circle_circle(pe_manifold_t *m)
{
    ...
    float r =
        m->af->shape.shape.circle.radius + m->bf->shape.shape.circle.radius;
    ...
    if (d != 0) {
        ...
        m->contacts[0] = VEC2F_ADD(
            VEC2F_MUL1(n, m->af->shape.shape.circle.radius), SHAPE_POS(m->af));
    } else {
