...
static int circle_in_poly(
    pe_manifold_t *m, pe_polygon_shape_t *poly, int normal_face)
{
    m->nb_contacts = 1;
    m->normal = pe_vec2f_opposite(pe_mat22_get_rotated_point(
        &m->bf->shape.mat_rot, poly->normals[normal_face]));
    m->contacts[0] =
        VEC2F_ADD(VEC2F_MUL1(m->normal, m->af->shape.shape.circle.radius),
            SHAPE_POS(m->af));
    ...
    pe_vec2f_t center, float *max_sep)
{
    ...
    for (int i = 0; i < poly->count; ++i) {
        sep = pe_vec2f_dot_product(
            poly->normals[i], VEC2F_SUB(center, poly->vertices[i]));
    ...
    face_vertices[1] =
        poly->vertices[(normal_face + 1 < poly->count) ? normal_face + 1 : 0];
    if (*max_sep < PE_EPSILON)
        circle_in_poly(m, poly, normal_face);
    return (*max_sep < PE_EPSILON) ? -2 : normal_face;
}

static int vorioni_region(
    pe_manifold_t *m, pe_vec2f_t vertex, pe_vec2f_t center)
{
    pe_vec2f_t normal;
    ...
    pe_vec2f_t face_vertices[2], pe_vec2f_t center)
{
    ...
    if (dot_products[0] <= 0.0f) {
        ...
        if (pe_vec2f_dot_product(VEC2F_SUB(center, face_vertices[0]), normal)
            > m->af->shape.shape.circle.radius)
            return 0;
        pe_mat22_rotate_point(&m->bf->shape.mat_rot, &normal);
...
char pe_manifold_fill_circle_polygon(pe_manifold_t *m)
{
    ...
    dot_products[0] = pe_vec2f_dot_product(VEC2F_SUB(center, face_vertices[0]),
        VEC2F_SUB(face_vertices[1], face_vertices[0]));
    dot_products[1] = pe_vec2f_dot_product(VEC2F_SUB(center, face_vertices[1]),
        VEC2F_SUB(face_vertices[0], face_vertices[1]));
