...
static int clip_face(pe_vec2f_t normal, float c, pe_vec2f_t *face)
{
    ...
    if (distances[0] * distances[1] < 0.0f) {
        ...
        out[separation] =
            VEC2F_ADD(face[0], VEC2F_MUL1(VEC2F_SUB(face[1], face[0]), alpha));
        separation++;
    }
    face[0] = out[0];
    face[1] = out[1];
...
static void fill_manifold(pe_manifold_t *m, pe_vec2f_t *inc_face, float ref_c,
...
static void get_ref_face(
    pe_manifold_t *m, pe_vec2f_t *ref_face, int ref_id, pe_shape_t *poly_a)
{
    int ref = ref_id;

    ref_face[0] = VEC2F_ADD(pe_mat22_get_rotated_point(&poly_a->mat_rot,
                                poly_a->shape.polygon.vertices[ref]),
        SHAPE_POS(m->af));
...
static void handle_collide(pe_manifold_t *m, int ref_id, int flip)
{
    ...
    if (clip_face(pe_vec2f_opposite(side_plane_normal), infos[1], inc_face)
        < 2)
        return;
...
char pe_manifold_fill_polygon_polygon(pe_manifold_t *m)
{
    ...
    float penetrations[2] = {
        find_axis_least_penetration(&faces[0], m->af, m->bf), 0};
