...
static pe_vec2f_t get_relative_contact_velocity(
    pe_vec2f_t *ra, pe_vec2f_t *rb, int i, pe_manifold_t *m)
{
...
void pe_resolve_collision_rotate(pe_manifold_t *m)
{
    ...
    for (int i = 0; i < m->nb_contacts; ++i) {
        ...
        if (contact_velocity > 0)
            return;
        impulse = VEC2F_MUL1(m->normal,
            (-(1 + m->e) * contact_velocity)
                / ((a->mass.inv_mass + b->mass.inv_mass
                       + powf(pe_vec2f_cross_product_vec_vec(ra, m->normal), 2)
                           * a->mass.inv_inertia
                       + powf(pe_vec2f_cross_product_vec_vec(rb, m->normal), 2)
                           * b->mass.inv_inertia)
                    * m->nb_contacts));
