...
void pe_body_apply_impulse_on_point(
    pe_body_t *body, pe_vec2f_t impulse, pe_vec2f_t contact_vec)
{
    body->velocity =
        VEC2F_ADD(body->velocity, VEC2F_MUL1(impulse, body->mass.inv_mass));
...
{
    body->velocity =
        VEC2F_SUB(body->velocity, VEC2F_MUL1(impulse, body->mass.inv_mass));
}
